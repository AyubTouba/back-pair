...
use crate::{
    db::{
        ...
        modules::{History, Profile},
        schema::{history, profiles},
    },
    dtos::history_dtos::{CreateHistroyDto, HistoryWithProfile},
};
...
pub fn list_history() -> Result<Vec<HistoryWithProfile>, Error> {
    ...
    let data = history::table
        ...
        .select((History::as_select(), Profile::as_select()))
        .order(history::created_at.desc())
        .load::<(History, Profile)>(connection)?;
    Ok(data
        .into_iter()
